...
class MyTimer : public Fl_Box {
    void draw() {
        ...
        fl_color(FL_BLACK);
        int i;
        for(i=0;i<sizeof(readings)/sizeof(*readings);i++){
            int x = w() / 2 + cos(i * M_PI * 2.0 / NUM_READINGS) * readings[i] * 0.9;
            int y = h() / 2 + sin(i * M_PI * 2.0 / NUM_READINGS) * readings[i] * 0.9;
            fl_rectf(x, y, 5, 5);
        }
    }
...
int main() {
    ...
    Fl::add_fd(fd, readdata, NULL);

    Fl_Double_Window win(700, 700);
