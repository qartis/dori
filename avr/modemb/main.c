...
ISR(USART_RX_vect)
{
    ...
    if (incoming_frametype > frametype) {
        ...
        if (type == TYPE_SMS && sms_cmd == FBUS_SMS_CMD_RECV) {
            ...
            for (i = 0; i < buf[FBUS_MSG_SIZE_OFFSET]; i++)
                sms_buf[i] = buf[FBUS_MSG_OFFSET + i];

            sms_buflen = buf[FBUS_MSG_SIZE_OFFSET];
...
{
    if (frametype == FRAME_SMS_RECV) {
        unbcd_phonenum(phone_num_buf);
        unpack7_msg(sms_buf, sms_buflen);
    }
